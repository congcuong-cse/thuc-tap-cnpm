...
def userGiangVien():
    ...
    group=Group.objects.get_by_natural_key('Giangvien')

    for gv in giangvien:
        user = User.objects.create_user(gv[0], gv[3], 'giangvien')
        user.save()
        user.groups.add(group)
        GiangVien.objects.create(mgv=user,ten_gv=gv[1],ngay_sinh=gv[2])
...
def userSinhVien():
    ...
    group=Group.objects.get_by_natural_key('Sinhvien')
    for sv in sinhvien:
        user = User.objects.create_user(sv[0], sv[6], 'sinhvien')
        user.groups.add(group)
        user.save()
        SinhVien.objects.create(mssv=user,ten_sv=sv[1],ngay_sinh=sv[2],gioi_tinh=sv[3],khoa=sv[4],nganh=sv[5])


def runHocKy():
    HocKy.objects.create(mhk='102',ten_hk='HK 2 (2010-2011)')
    HocKy.objects.create(mhk='111',ten_hk='HK 1 (2011-2012)')
    HocKy.objects.create(mhk='112',ten_hk='HK 2 (2011-2012)')
